...
move(X,Y,Z):- K is X+Y, K<4, Y>0, \+member((K,0),Z), write("Pour some from 4l jug"), nl, move(K,0,[(K,0)|Z]).
move(X,Y,Z):- K is X+Y, K<3, X>0, \+member((0,K),Z), write("Pour some from 3l jug"), nl, move(0,K,[(0,K)|Z]).